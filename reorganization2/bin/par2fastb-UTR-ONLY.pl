...
#=====================================================================
die "par2fastb.pl <genome.2bit> <indir> <outdir> <lib1> <lib2> ...\n" unless @ARGV>=4;
my $TWO_BIT_FILE=shift @ARGV;
my $DATASETS_SUBDIR=shift @ARGV;
...
foreach my $lib (@libs) {
  my $filename="$DATASETS_SUBDIR/$lib.clusters.csv";
...
foreach my $libraryID (@libs) {
  ...
  open(IN,$groupsFile) || die "can't open file: $groupsFile";
  while(<IN>) {
    chomp;
    my @fields=split/,/,$_;
    my $groupID=$fields[4];
